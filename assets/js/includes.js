(()=>{
  ...
  async function inject(selector,file){
    ...
    try{const response=await fetch(root+"includes/"+file+"?v=8",{cache:"no-cache"});if(!response.ok)throw new Error(response.status);host.innerHTML=(await response.text()).replaceAll("{{ROOT}}",root)}
